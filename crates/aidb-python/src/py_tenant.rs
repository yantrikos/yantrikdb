...
use crate::py_engine::{map_err, PyAIDB};
...
/// Python multi-tenant manager. Creates isolated PyAIDB instances per tenant.
///
...
impl PyTenantManager {
    ...
    /// Get a PyAIDB instance for a tenant.
    ///
    ...
    fn get(&self, tenant_id: &str, embedder: Option<PyObject>) -> PyResult<PyAIDB> {
        let db_path = self
            .base_dir
        ...
        let inner = match config.and_then(|c| c.encryption_key.as_ref()) {
            Some(key) => AIDB::new_encrypted(&db_path, dim, key).map_err(map_err)?,
            None => AIDB::new(&db_path, dim).map_err(map_err)?,
        };
