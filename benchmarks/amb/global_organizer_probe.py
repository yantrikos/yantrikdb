...
def main() -> int:
    ...
    prompt = (
        ...
        "relationship roles as hard clustering evidence: family or friends, academic "
        "advisors, industry mentors or peers, and service providers should not share "
        "a handle merely because they discuss the same project. A multi-person "
        "handle is appropriate only when the people are explicitly connected or "
        "repeatedly participate in one shared event arc. Keep corrections, denials, "
        "and superseded claims visibly separate from the active event timeline; do "
        "not let a contradiction become another positive milestone. Use "
